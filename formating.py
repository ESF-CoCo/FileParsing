...
front_folder_path = "PartFront"
back_folder_path = "PartBack"

# Get a list of all image files in the front and back folders
front_image_files = sorted([f for f in os.listdir(front_folder_path) if f.endswith('.png')])[:82]
back_image_files = sorted([f for f in os.listdir(back_folder_path) if f.endswith('.png')])[:82]
...
for page_num in range(num_pages):
    # Create front page
    front_page_buffer = create_page(front_image_files, front_folder_path, page_num, is_front=True)
    front_pdf_reader = PyPDF2.PdfReader(front_page_buffer)
    output_pdf.add_page(front_pdf_reader.pages[0])
    
    # Create back page
    back_page_buffer = create_page(back_image_files, back_folder_path, page_num, is_front=False)
